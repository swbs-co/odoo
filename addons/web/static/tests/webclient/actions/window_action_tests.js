...
QUnit.module("ActionManager", (hooks) => {
    ...
    QUnit.test("breadcrumbs are updated when display_name changes", async function (assert) {
        ...
        await testUtils.dom.click(target.querySelector(".o_control_panel .o_form_button_save"));
        assert.containsN(
            target,
            ".o_control_panel .breadcrumb-item",
            2,
    ...
    <button string="Cancel" class="cancel-btn" special="cancel"/>
            ...
            await legacyExtraNextTick();
            assert.ok(
                target.querySelector(".o_cp_buttons .o_form_button_edit").disabled,
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            assert.ok(target.querySelector(".o_cp_buttons button").disabled);
            await nextTick();
            await legacyExtraNextTick();
            assert.notOk(target.querySelector(".o_cp_buttons button").disabled);
        }
    );
    ...
    QUnit.test("can open different records from a multi record view", async function (assert) {
        ...
        await testUtils.dom.click($(target).find(".o_list_view .o_data_row:nth(1)"));
        await legacyExtraNextTick();
        assert.strictEqual(
            $(target).find(".o_control_panel .breadcrumb-item:last").text(),
            "Second record",
    ...
    QUnit.test("restore previous view state when switching back", async function (assert) {
        ...
        await doAction(webClient, 3);
        assert.hasClass(
            $(target).find(".o_control_panel  .fa-bar-chart-o")[0],
            "active",
            "bar chart button is active"
        );
        assert.doesNotHaveClass(
            $(target).find(".o_control_panel  .fa-area-chart")[0],
            "active",
            "line chart button is not active"
        );
        // display line chart
        await testUtils.dom.click($(target).find(".o_control_panel  .fa-area-chart"));
        await legacyExtraNextTick();
        assert.hasClass(
            $(target).find(".o_control_panel  .fa-area-chart")[0],
            "active",
    ...
    QUnit.test("can open a many2one external window", async function (assert) {
      ...
      <form>
        ...
        await testUtils.dom.click($(target).find(".o_external_button"));
        await legacyExtraNextTick();
        assert.verifySteps([
            "/web/webclient/load_menus",
            "/web/action/load",
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            await testUtils.dom.clickLast($(target).find(".o_control_panel .breadcrumb a"));
            await legacyExtraNextTick();
    ...
    QUnit.test("honor group_by specified in actions context", async function (assert) {
      ...
      <search>
        ...
        await testUtils.dom.click(
            $(target).find(".o_control_panel .o_cp_bottom_right button:contains(Group By)")
        );
        await testUtils.dom.click(
            $(target).find(".o_control_panel .o_group_by_menu .o_menu_item:first")
        );
        await legacyExtraNextTick();
    ...
    QUnit.test("save current search", async function (assert) {
        ...
        const webClient = await createWebClient({ target, serverData, legacyParams });
        await doAction(webClient, 33);
    ...
    QUnit.test(
        ...
        async function (assert) {
            ...
            assert.strictEqual(
                $(target).find(".o_list_view tr.o_data_row .o_data_cell").text(),
                "zoupyopplopgnapblip",
            ...
            await legacyExtraNextTick();
            assert.strictEqual(
                $(target).find(".o_control_panel .o_facet_values").text().trim(),
                "favorite filter",
    ...
    QUnit.test("execute action from dirty, new record, and come back", async function (assert) {
      ...
      <form>
        ...
        await testUtils.fields.editInput($(target).find('input[name="display_name"]'), "test");
        await testUtils.fields.editInput($(target).find("input[name=foo]"), "val");
        await testUtils.dom.click($(target).find(".o_form_uri:contains(First record)"));
        await legacyExtraNextTick();
    ...
    QUnit.test("execute a contextual action from a form view", async function (assert) {
        ...
        await testUtils.controlPanel.toggleMenuItem(target, "Favorite Ponies");
        await legacyExtraNextTick();
        assert.containsOnce(target, ".o_list_view");
    });
