...
class CrmLead(models.Model):
    ...
    def action_new_quotation_handle_partner(self):
        if not self.partner_id:
            ...
            return self.new_quotation_action()
    ...
    def _get_base_view_order_action(self, filter_state=[]):
        action = {
            'context': {
                'search_default_draft': 'draft' in filter_state,
                'search_default_partner_id': self.partner_id.id,
                'default_partner_id': self.partner_id.id,
                'default_opportunity_id': self.id
            },
            'domain': [('opportunity_id', '=', self.id), ('state', 'in', filter_state)]
        }
        orders = self.mapped('order_ids').filtered(lambda l: l.state in filter_state)
        if len(orders) == 1:
